...
void DataBaseModel::updateAllMaxes() {
  int newLongestDistance =
      *(std::max_element(m_weekMeters.begin(), m_weekMeters.end()));
  if (newLongestDistance > longestDistance()) {
    setLongestDistance(newLongestDistance);
  }

  int newLongestDuration =
      *(std::max_element(m_weekTime.begin(), m_weekTime.end()));
  if (newLongestDuration > longestDuration()) {
    setLongestDuration(newLongestDuration);
  }

  int newweeklyMetersRun =
      std::accumulate(m_weekMeters.begin(), m_weekMeters.end(), 0);
  ...
  for (int i = 0; i < 7; ++i) {
    if (m_weekTime[i] > 0)
      newBestPace = std::max(m_weekMeters[i] / m_weekTime[i], newBestPace);
  }
  if (newBestPace > bestPace()) {
    setBestPace(newBestPace);
  }

  int newAverageDuration =
      std::accumulate(m_weekTime.begin(), m_weekTime.end(), 0) / 7;
...
void DataBaseModel::initializeDataBase() {
  ...
  if (db.isOpen()) {
    db.clearDataBase();
    db.createTable();
    db.addElement("Monday", m_weekMeters[0],
                  m_weekTime[0]); // day, meters, time
    ...
    db.addElement("Goal", m_weeklyGoal,
                  m_dailyGoal); // this is ugly, but I don't have to use
                                // Monday_km and so on
...
void DataBaseModel::insertDataToDatabase(
...
void DataBaseModel::setweeklyMetersRun(int newweeklyMetersRun) {
  ...
  insertDataToDatabase("weeklyMetersRun_BestPace", m_weeklyMetersRun,
                       m_bestPace, 11);
  emit weeklyMetersRunChanged();
}
...
void DataBaseModel::setMail(QString newMail) {
  ...
  if (db.isOpen()) {
    db.updateMail(
        "Mail", newMail,
        12); // todo this won't work, see updateMail function in database.cpp
    // qDebug() << "setmail";
  } else
    qDebug() << "Database is not open!";
  emit mailChanged();
...
QVector<int> DataBaseModel::weekMeters() const { return m_weekMeters; }
QVector<int> DataBaseModel::weekTime() const { return m_weekTime; }